...
class LLMService:
    ...
    def _build_voiceover_context(
        ...
        return (
            ...
            "事实约束：如果主题要求 TOP8/TOP10 或多个项目，必须基于参考资料里的真实项目名、链接或描述来写；"
            "参考资料没有给出具体项目时，不要编造项目名、Star 数、增长率、生产环境案例或具体排名；"
            "不要用“模型压缩工具”“自动化工作流”这类泛泛类别冒充具体项目；"
            "缺少项目清单时，请写成选题框架，并明确提示需要补充项目清单。\n\n"
    ...
    def _build_scene_plan_context(
        ...
        return (
            ...
            "分镜只能拆解口播稿已经出现的信息，不能额外补项目名、数据或技术细节。\n\n"
            f"主题：{topic or 'DevShorts AI'}\n"
